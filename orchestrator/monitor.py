...
При устойчивой недоступности ноды шлём владельцу алерт. Недоступность — это
любой статус, кроме online и disabled (disabled оператор выключил сам; offline
и connecting означают, что нода не на связи — при падении панель может надолго
застрять в connecting, бесконечно переподключаясь). Чтобы моргание при ребуте
VPS не порождало ложную тревогу, алерт срабатывает только после
OFFLINE_ALERT_THRESHOLD подряд таких опросов (~15 минут реальной недоступности
при шаге 5 минут). Когда нода возвращается, шлём короткое «снова online».
Счётчики живут в ctx воркера между запусками cron; перезапуск воркера их
сбрасывает — это лишь отложит, но не потеряет алерт.
...
async def _poll_one(
    *,
    ...
    # «Нода недоступна» — это любой статус, кроме online и disabled. disabled
    # выключена оператором намеренно (не повод для тревоги), а connecting при
    # реальном падении ноды панель может держать сколь угодно долго (бесконечный
    # реконнект по gRPC) — поэтому его, как и offline, считаем за недоступность,
    # иначе об упавшей «вечно connecting» ноде алерт бы не пришёл никогда.
    if status not in (NodeConnState.ONLINE, NodeConnState.DISABLED):
        ...
        if counts[node.id] >= OFFLINE_ALERT_THRESHOLD and node.id not in alerted:
            ...
            await _send(
                ...
                f"(недоступна уже ~{counts[node.id] * POLL_EVERY_MINUTES} мин).",
            )
        return

    # Доступна (online) или выключена (disabled): сбрасываем счётчик и, если по
    # ноде был алерт, сообщаем о возврате — но только при честном online.
    counts[node.id] = 0
